-- One row per non cancelled AR invoice with what has been received against it
...
with inv as (
    select * from {{ ref('stg_ar_invoice') }}
    where status <> 'CANCELLED'
),
app as (
    select invoice_id,
           sum(received_amount) as applied_amount,
           max(receipt_date)  as last_applied_date
    from {{ ref('stg_ar_receipt') }}
    group by invoice_id
),
fx as (
...
select
    ...
    applied_amount::number(18,2)                            as received_amount,
    outstanding_amount::number(18,2)                        as outstanding_amount,
    last_applied_date                                       as last_receipt_date,
